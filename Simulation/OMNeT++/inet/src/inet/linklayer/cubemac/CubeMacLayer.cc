...
void CubeMacLayer::initialize(int stage)
{
    ...
    if (stage == INITSTAGE_LOCAL) {
        ...
        // --- Medium sensing here? Neighbour collision announcement?
...
void CubeMacLayer::findNewSlot()
{
    ...
    while ((occSlotsAway[mySlot] != CUBEMAC_FREE_SLOT) && (counter < (numSlots - reservedMobileSlots))) {
        counter++;
        mySlot--;
        if (mySlot < 0)
            mySlot = (numSlots - reservedMobileSlots) - 1;
    }
    if (occSlotsAway[mySlot] != CUBEMAC_FREE_SLOT) {
